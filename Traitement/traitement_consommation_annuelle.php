...
if (!isset($_SESSION['loggedIn']) || $_SESSION['loggedIn'] !== true) {
    header("Location: dashboard.php?message=Accès non autorisé - Session invalide");
    exit();
}

// Vérification du rôle client (ID_Role = 1)
if (!isset($_SESSION['user_role']) || $_SESSION['user_role'] != 2) {
    header("Location: dashboard.php?message=Accès réservé aux agents");
    exit();
}

if (!isset($_SESSION['user_id'])) {
    header("Location: dashboard.php?message=Identifiant client manquant");
    exit();
}
...
if (!$pdo) {
    header("Location: dashboard.php?message=Connexion DB impossible");
    exit();
}
...
if (!$AgentId) {
    header("Location: dashboard.php?message=Agent avec l'ID_Utilisateur $UserId n'existe pas.");
    exit();
}
...
if (isset($_POST['submit'])) {
    ...
    if ($fileExt !== 'txt') {
        header("Location: dashboard.php?message=Erreur: Veuillez télécharger un fichier de type .txt.");
        exit();
    } 
    ...
    if (empty($matches)) {
        header("Location: dashboard.php?message=Erreur: Le nom du fichier doit suivre le format 'consommation_annuelle_(annee).txt'.");
        exit();
    }
    ...
    header("Location: ../IHM/agent_dashboard.php?message=Fichier traité avec succès!");
    exit();
}
?>
